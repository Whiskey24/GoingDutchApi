...
switch($mode){
  ...
  case "validate":
    ...
    if ($_POST['deposit_holder']) {
      if (!in_array($_POST['deposit_holder'],$member_ids)) {
    ...
    if(!empty($errorString)) {
      ...
      if ($_POST['deposit_holder']) $xuid = $_POST['deposit_holder'];
      else $xuid = $user->data['user_id'];
      
      
      // deposit, so type = 0
      $type = 0;
      if (book_expense($_POST['description'],$_POST['amount'],$type,$timestamp,$xuid,$groupdetails['group_id'],$_POST['members'],$_POST['event_id'],1)) {
...
if ($showaddform) {
  ...
  if ( array_key_exists(4, $permissions) ) {
    ...
    if (isset($_POST['deposit_holder'])) $selected = $_POST['deposit_holder'];
    else $selected = $user->data['user_id'];
    
    $formarray['rows'][0]['type'] = "select";
    $formarray['rows'][0]['label'] = "Deposit holder: ";
    //* $formarray['rows'][0]['name'] = "deposit_holder";
    $formarray['rows'][0]['name'] = "deposit_holder";
  ...
  $formarray['rows'][3]['items'] = "Amount each:|amount|text|" . $_POST['amount'];;
  /*
  ...
  */
    ...
    $formarray['rows'][4]['type'] = "select";
    $formarray['rows'][4]['html'] = $selecthtml;
 
    $formarray['rows'][5]['type'] = "textrow";
    $formarray['rows'][5]['label'] = "Participants in deposit";
  ...
  $formarray['rows'][9]['items'] = "||submit|Add deposit";
